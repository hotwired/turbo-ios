...
Turbo for iOS is written in Swift 5.3 and requires Xcode 12. It currently supports iOS 12 or higher, but we'll most likely drop iOS 12 support soon.
...
Install Turbo manually by building `Turbo.framework` and linking it to your project.
...
This repository includes a demo app to show off features of the framework. To run the demo, clone this repo to your computer and change into its directory. Then, run the demo app by opening `Demo/Demo.xcworkspace` in Xcode and run the Demo target. By default, the demo app loads a remotely hosted Turbo demo app. You can also clone that demo app to experiment. It's hosted here - [https://github.com/basecamp/turbo-native-demo](https://github.com/basecamp/turbo-native-demo), and then you just need to change `Demo.current` to use `local` in `Demo/Demo.swift`. Then it will use the local version of the demo.
...
# Understanding Turbo Native Concepts

The `Session` class is the central coordinator in a Turbo Native for iOS application. It creates and manages a single WKWebView instance, and lets its delegate—your application—choose how to handle link taps, present view controllers, and deal with network errors.
...
Turbo for iOS is open-source software, freely distributable under the terms of an [MIT-style license](LICENSE). The [source code is hosted on GitHub](https://github.com/turbolinks/turbolinks-ios).
